# Band Quest — Character Portrait Generation Prompts

These are ready-to-use prompts for generating the **11 class portraits** with an
image model (Midjourney, DALL·E 3, Imagen, SDXL, Nano Banana, etc.). Each portrait
is the hero art for one playable class. Generate, then drop the result into
`bandquest/public/portraits/<instrument>.png` and the app picks it up automatically
(see `docs/PORTRAITS.md`).

---

## Shared style preamble (paste this in front of EVERY class prompt)

> High-quality hand-painted 2D anime/JRPG character portrait, in the combined style of
> **One-Punch Man's** confident expressive linework and **Final Fantasy VI / VII Remake's**
> atmospheric painterly rendering. Bust / upper-body framing, character facing slightly
> 3/4 toward the viewer. **Highly detailed, realistically structured face**: defined nose
> with visible bridge and nostrils, full lips with distinct upper and lower lip and a soft
> philtrum, sculpted chin and jawline, soft cheek and jowl modelling, individually rendered
> eyelashes, shaped eyebrows with hair detail, and deep expressive eyes with iris texture,
> catchlights, and a subtle limbal ring. **Painterly soft shading with clear contour lines,
> form shadows, ambient occlusion, rim lighting, and a defined single key light** from the
> upper left. Smooth skin gradients, not flat fills. Subtle subsurface warmth in the skin.
> Clean dramatic three-quarter lighting. Centered composition, head-and-shoulders, looking
> at or just past the camera. Plain softly-vignetted background that does not distract from
> the character. No text, no watermark, no border, no UI. Consistent roster: all portraits
> share the same rendering quality, lighting direction, and framing so they look like one
> cast.

Then append ONE of the per-class blocks below.

---

## 1. Flute — **Sylph** (Elf Ranger / Healer)
**Reference: Aerith Gainsborough (Final Fantasy VII).**

> A graceful young woman wind mage-healer, **strongly inspired by Aerith from Final Fantasy VII**:
> warm hazel-green eyes, long wavy chestnut-brown hair gathered back with a pink ribbon and a
> few loose framing strands, gentle hopeful expression with a faint warm smile. She wears a
> light flowing ranger's dress in soft sage-green and cream with a cropped layered bodice and
> delicate silver leaf-motif clasps. Around her, faint floating motes of light like pollen.
> She holds a **silver concert flute** across her chest, gleaming with a cool highlight,
> fingers resting on the keys. Cool woodwind palette — sage green, silver, soft rose accents.
> Kind, ethereal, life-giving presence.

---

## 2. Clarinet — **Fugulist** (Martial Arts Master)
**Reference: Air Nomads / wind village members, Avatar: The Last Airbender.**

> A poised martial-arts wind monk, **strongly inspired by the Air Nomads of Avatar: The Last
> Airbender**: warm light-brown skin, calm focused eyes, shaved or close-cropped head OR hair
> swept back simply, serene disciplined expression. Flowing layered robes in **saffron-orange,
> warm yellow, and deep brown** with wide wrapped sleeves, a sash belt, and arrow-like
> tattoo/trim motifs echoing airbender garb (stylized, not a direct copy). Light, agile build
> suggesting speed and precision. He/she holds a **black grenadilla-wood clarinet with silver
> keys** in a ready stance, as if it were a staff/weapon. Swirls of stylized wind curl around
> the sleeves. Cool-meets-warm palette anchored by saffron and brown. Tranquil but coiled,
> ready to move.

---

## 3. Alto Saxophone — **Spectre** (Shapeshifter / Mimic Mage)
**Reference: Magus (Chrono Trigger).**

> A brooding arcane shapeshifter, **strongly inspired by Magus from Chrono Trigger**: very pale
> cool lavender-toned skin, long flowing silver-blue hair, sharp angular eyebrows, intense
> crimson-red eyes, stern thin-lipped confident expression. He wears **dark purple-black robes
> with a high collar** and a long flowing **navy-purple cape** clasped with cold-purple shoulder
> clasps, a glowing arcane rune on the chest. Cold violet magical aura. He holds a **brass alto
> saxophone**, its keys wreathed in a faint purple magical glow. Dramatic, dark, cold palette —
> deep purples, silver-blue, crimson accents. Imposing, mysterious, powerful.

---

## 4. Trumpet — **Vanguard** (Soldier / Military Leader)
**Reference: Paladin Cecil / the Paladins of Final Fantasy IV.**

> A noble armored knight-commander, **strongly inspired by the Paladins of Final Fantasy IV
> (Paladin Cecil)**: resolute heroic face, determined eyes, light hair (silver or pale blond)
> framing the face, dignified courageous expression. He wears **gleaming silver-and-gold plate
> armor** with ornate pauldrons, a high gorget, and a flowing cape; a soft holy light rims his
> silhouette. Brass/gold warm palette with steel-blue shadows. He raises a **polished golden
> trumpet** like a signal-horn / weapon, the bell catching a bright highlight. Radiant, valiant,
> a born leader. Warm brass palette — gold, bronze, ivory, with cool steel accents.

---

## 5. Trombone — **Slide Knight** (attack-focused Paladin)
**Reference: Metal Knight (Bofoi) from One-Punch Man.**

> A towering mechanical warrior in fully enclosed **heavy combat armor**, **strongly inspired by
> Metal Knight from One-Punch Man**: no visible human skin — the entire form is a suit of
> thick plated armor, dark gunmetal-grey with red sensor lenses glowing in the helmet's eye
> slots, chunky angular pauldrons, layered chest and arm plates, heavy armored gauntlets.
> The armor suggests immense destructive capability held in perfect cold control. A **gold
> trombone** is built into one forearm like a weapon port, slide extending mechanically outward.
> Dark steel and deep red palette with glowing accent lines. Imposing, mechanical, zero
> humanity visible — the weapon wears the warrior.

---

## 6. Euphonium — **Resonant** (Cleric-Knight)
**Reference: Metal Knight (Bofoi) from One-Punch Man — heavier support-suit variant.**

> A massive fully-armored support combat suit, **strongly inspired by Metal Knight from
> One-Punch Man** but in a heavier, more fortified configuration: completely enclosed
> gunmetal-dark armor plating, red sensor lenses, enormous rounded pauldrons, a broad
> reinforced chest with a single glowing warm-gold sigil panel (the healing array). Chunky
> armored hands cradle a **compact gold euphonium** like a power source / sacred core.
> Where the Trombone Slide Knight reads as a weapons platform, this reads as a **fortress**:
> the same mechanical identity but built for endurance and protection. Dark iron and deep gold
> palette, protective energy aura around the instrument. Immovable, fortified, shielding.

---

## 7. Percussion — **Rhythm Forger** (Mechanic / Gadget-Maker)
**Reference: Lucca Ashtear from Chrono Trigger.**

> A brilliant young inventor-warrior, **strongly inspired by Lucca from Chrono Trigger**:
> large round spectacles / goggles (either worn or pushed up on the forehead), vivid
> **purple hair** in a high tied bun with flyaway strands, warm olive-tan skin, sharp
> clever eyes, a wide confident grin. She wears a **layered inventor's coat with drumhead
> panels on the chest, buckles and gadget pouches at the belt, and reinforced bracers**,
> in purple and dark metal tones echoing Lucca's colors. Holds a pair of **drumsticks**
> crossed in front like weapons, a compact mallet rig at her hip. Around her, faint
> heat-shimmer sparks of orange and red — a fire-element inventor's energy. Purple, dark
> metal, and warm fire-orange accent palette. Smart, scrappy, dangerous, delighted to be here.

---

## 8. French Horn — **Forest Caller** (Support Mage / Druid)

> A serene nature-attuned druid-mage. Soft thoughtful features, leaf or antler motifs in the
> hair, gentle knowing eyes. Flowing **green-and-brown layered druidic robes** with vine and
> bark textures, a cloak of leaves. Faint echoing sound-rings ripple around them. Cradles a
> coiled **gold French horn** like a hunting horn of the deep woods. Cool forest palette —
> mossy greens, bark browns, soft gold. Calm, mystical, far-traveling.

---

## 9. Tuba — **Brass Bastion** (Heavy Armored Tank)
**Reference: Brick from The Incredibles 2.**

> A powerfully built superhero-tank, **strongly inspired by Brick from The Incredibles 2**:
> a large, broad-shouldered, muscular young woman with a wide flat face, a friendly-but-
> immovable expression, small calm eyes, and short blunt hair. She wears **enormous layered
> brass-and-iron plate armor** scaled to her frame — massive pauldrons, a deep gorget, heavy
> gauntlets — but with a superhero-suit undercurrent: the armor is functional, purposeful,
> built around her body rather than constraining it. A gigantic **gold tuba** rests over one
> shoulder like a siege cannon, its great bell flared toward the viewer. Deep brass and dark
> iron palette with dim inner glow. Unshakeable, immovable, completely unfazed.

---

## 10. Oboe — **Crystal Mystic** (Attack Mage)
**Reference: Whis from Dragon Ball Super.**

> An ethereal angel-like arcane mage of perfect poise, **strongly inspired by Whis from Dragon
> Ball Super**: extremely slender graceful build, very tall and elegant, **pale lavender-white
> skin**, **long silver-white hair** draped around the shoulders, calm half-lidded eyes with
> a thin knowing smile, an air of effortless superiority and ancient wisdom. He wears
> **layered long robes of crystal-blue and white with a high decorated neckpiece**, a pale
> blue marking on the forehead, and holds a slender **blackwood oboe with silver keys** as
> though it were his ornate staff — poised vertically, perfectly still. Faint crystalline
> shards and precise light-refractions orbit him. Cool jewel palette: crystal blue, white,
> silver, lavender, with pinpoint prismatic highlights. Serene, ageless, effortlessly
> overwhelming.

---

## 11. Bassoon — **Augur** (Scientist / Disruptor)
**Reference: Melchior (Guru of Life) from Chrono Trigger.**

> An ancient white-bearded sage-craftsman, **strongly inspired by Melchior from Chrono
> Trigger**: deeply wrinkled kind face, long flowing **white hair and beard**, warm wise eyes
> behind small half-glasses, a gentle knowing expression that conceals profound knowledge of
> acoustic science and harmonic theory. He wears **layered dark scholar's robes** with arcane
> instruments, vials of resonant material, and a battered research satchel; faint floating
> runic diagrams and harmonic-frequency rings drift around him. He holds a long dark **bassoon**
> as a staff — its bell on the ground, both hands resting on the body. Muted earthy palette:
> charcoal, oxblood, aged brass, deep teal. Patient, ancient, devastatingly precise.

---

## Tips for a consistent roster

- Generate at a **square or 4:5 aspect ratio**, then crop to a consistent head-and-shoulders.
- Keep the **same key-light direction (upper left)** across all 11 so the cast matches.
- If your model supports it, reuse a **style/character-reference** image of the first finished
  portrait you like as a seed for the rest to lock in rendering quality.
- Export PNG (transparent background preferred). Target ~1024×1024 or larger; the app
  downscales cleanly.
- Filenames must match the instrument id exactly:
  `flute, clarinet, alto_sax, trumpet, trombone, euphonium, percussion, french_horn, tuba, oboe, bassoon`.
